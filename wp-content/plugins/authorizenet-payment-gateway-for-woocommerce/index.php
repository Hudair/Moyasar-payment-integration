      ...
      **/
      function check_authorize_response()
      {
         ...
         if ( count($_POST) ){
            ...
            if ( $_POST['x_response_code'] != '' &&  ( strtoupper($_POST['x_SHA2_Hash']) !=  '' ) ){
               try{                  
      ...
      **/
      public function generate_authorize_form($order_id)
      {
         ...
         if( $this->working_mode == 'false' ){
            $authorize_args['x_solution_id'] = 'AAA172029';
         }
        ...
        if($this->mode == 'true'){
         ...
         $html_form    = '<form action="'.$processURI.'" method="post" id="authorize_payment_form">' 
               ...
               . '<script type="text/javascript">
                  jQuery(function(){
                     jQuery("body").block({
                           message: "<img src=\"'.plugin_dir_url( __FILE__ ).'images/loader.gif\" alt=\"Redirecting…\" style=\"float:left; margin-right: 10px; width:48px;\" />'.__('Thank you for your order. We are now redirecting you to Authorize.net to make payment.', 'tech').'",
                           overlayCSS:
                        {
                           background:       "#ccc",
                           opacity:          0.6,
...
$utils  = new Indatos_Authorizenetbasic_Utils();
